...
//#define MAXINPUTNO 8	/* データの最大個数 */
//
///* 関数のプロトタイプ宣言 */
//double f(double u);								/* 伝達関数 */
//void initwh(double wh[HIDDENNO][INPUTNO + 1]);	/* 中間層の重みの初期化 */
//void initwo(double wo[HIDDENNO + 1]);			/* 出力層の重みの初期化 */
//double forward(double wh[HIDDENNO + 1][INPUTNO + 1], double wo[HIDDENNO + 1], double hi[], double w[INPUTNO]);	/* 順方向の計算 */
...
//int getdata(double e[][INPUTNO])
//{
//	int n_of_e = 0;  /* データのセットの個数 */
//	int j = 0;       /* 繰り返しの制御用 */
//
//	/* データの入力 */
//	while (n_of_e < MAXINPUTNO)
//	{
//		if (scanf("%lf", &e[n_of_e][j]) != 1)
//		{
//			// 数値の読み取りに失敗した場合やEOFの場合はループを終了
//			break;
//		}
//		j++;
//		if (j >= INPUTNO)
